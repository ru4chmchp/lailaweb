                        ...
                        <form action="{{ route('settings.update', ['id' => $settings->id]) }}" method="POST">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control @error('config_key') is-invalid @enderror"
                                    placeholder="Nhập Config Key" name="config_key" value="{{ $settings->config_key }}">
                                @error('config_key')
                            ...
                            @elseif (request()->type === 'Textarea')
                                <div class="form-group">
                                    <label>Config Value</label>
                                    <textarea name="config_value @error('config_value') is-invalid @enderror" rows="4" placeholder="Nhập Config Value"
                                        class="form-control">{{ $settigns->config_value }}</textarea>
                                </div>
                                @error('config_value')
                                    <div class="alert alert-danger m-2">{{ $message }}</div>
                                @enderror
